...
message-type : ∀ {To} → Message To → MessageType
message-type (Msg {MT} _ _) = MT 

add-references : ∀ {To} → TypingContext → Message To → TypingContext
add-references Γ msg = add-references-static Γ (message-type msg)
...
selected-type : ∀ {IS}
                    ...
                    MessageType
selected-type record {msg = msg } = message-type msg

add-selected-references : TypingContext → ∀ {IS}
                            {filter : MessageFilter IS} →
                            SelectedMessage filter → TypingContext
add-selected-references Γ m = add-references-static Γ (selected-type m)
\end{code}
%</SelectedMessageAddReferences>
\begin{code}
...
return₁ : {A : Set (suc zero)} → ∀ {i IS post} → (val : A) → ∞ActorM i IS A (post val) post
