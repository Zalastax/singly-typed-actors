...
⊆++-refl : ∀ {a} {A : Set a} → (xs ys : List A) → xs ⊆ (xs ++ ys)
⊆++-refl [] ys = []
...
move-received : ∀ {IS} → ∀ pre → (q : List (Message IS)) → (x : Message IS) → ((pre ++ (waiting-refs (q ++ (x ∷ [])))) ⊆ (add-references (pre ++ waiting-refs q) x))
move-received pre q (Msg {MT} x x₁) rewrite (waiting-refs++ q (Msg x x₁ ∷ [])) | (++-identityʳ (extract-references MT)) = ⊆-trans move-1 move-2
  where
    move-1 : (pre ++ waiting-refs q ++ extract-references MT) ⊆ ((pre ++ waiting-refs q) ++ extract-references MT)
    move-1 = ⊆++comm' pre (waiting-refs q) (extract-references MT)
    move-2 : ((pre ++ waiting-refs q) ++ extract-references MT) ⊆ (extract-references MT ++ (pre ++ waiting-refs q))
...
accept-found pre q record { heads = heads ; el = Msg {MT} x y ; tails = tails ; is-ls = is-ls } rewrite (sym is-ls) =
  ...
  final-move))
  where
    final-move : (extract-references MT ++ waiting-refs (heads ++ tails)) ⊆ waiting-refs (heads ++ Msg x y ∷ tails)
    final-move rewrite (waiting-refs++ heads tails) | (waiting-refs++ heads (Msg x y ∷ tails)) = ⊆-trans (⊆++comm' (extract-references MT) (waiting-refs heads) (waiting-refs tails))
