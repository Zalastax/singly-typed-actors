...
data ActorM (IS : InboxShape) : (A : Set₁) →
  ...
  SelectiveReceive : ∀ {pre} → (filters : List (MessageFilter IS)) → ActorM IS (Σ[ m ∈ (Message IS) ] Σ[ filter ∈ MessageFilter IS ] Σ[ which ∈ (filter ∈ filters) ] filter m ≡ true) pre λ { (m , rest) → add-references pre m }
