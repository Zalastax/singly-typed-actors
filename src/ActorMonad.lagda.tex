...
Agda will solve the constraints, and the value or reference will be accessible.
...
For the sender it's a guarantee that every type-correct message can be sent,
and for the receiver it's a guarantee that no type-incorrect message will be received.
Using the analogy of \AgdaRef[ActorMonad]{InboxShape} being a set,
a reference type is compatible with another if the actual \AgdaRef[ActorMonad]{InboxShape} is a superset of the wanted \AgdaRef[ActorMonad]{InboxShape}.

\begin{code}
record _<:_ (actual wanted : InboxShape) : Set₁ where
  field
