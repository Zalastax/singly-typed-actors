...
it became apparent that an embedding in Agda would have a clear advantage over a simple λ-calculus.
...
monad, which you can read about in section \ref{parameterized_monads}.
...
We don't give the Return constructor the name return,
since we want return to be a coinductive \AgdaRef[ActorMonad]{ActorM} and 
the constructors of \AgdaRef[ActorMonad]{ActorM} can't return an $∞ ActorM$.
Instead, the actual return is defined as a separate function that wraps Return in a thunk.
